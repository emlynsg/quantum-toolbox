...
/// Currently need to add wavefunctions first
  ...
  system.addZeroWavefunction(mu, epsilons[1]);
  system.addZeroWavefunction(mu, epsilons[1]);
  system.addZeroWavefunction(mu, epsilons[2]);
  system.addZeroWavefunction(mu, epsilons[2]);
  system.addZeroWavefunction(mu, epsilons[2]);
  system.addZeroWavefunction(mu, epsilons[2]);
  for (int k = 7; k < 13; ++k) {
    system.addZeroWavefunction(mu, epsilons[3]);
  }
  system.addGaussianPotential(0.0, V0, sigma, 1, 1);
  system.addGaussianPotential(0.0, V0, sigma, 2, 2);
  system.addGaussianPotential(0.0, V0, sigma, 3, 3);
  system.addGaussianPotential(0.0, V0, sigma, 4, 4);
  system.addGaussianPotential(0.0, V0, sigma, 5, 5);
  system.addGaussianPotential(0.0, V0, sigma, 6, 6);
  for (int k = 7; k < 15; ++k) {
    system.addGaussianPotential(0.0, V0, sigma, k, k);
  }


  system.addGaussianPotential(couplingCentre, F, sigmaF, 0, 1);
  system.addGaussianPotential(couplingCentre, F, sigmaF, 1, 0);
  system.addGaussianPotential(couplingCentre, F, sigmaF, 0, 2);
  system.addGaussianPotential(couplingCentre, F, sigmaF, 2, 0);
  system.addGaussianPotential(couplingCentre, F, sigmaF, 3, 1);
  system.addGaussianPotential(couplingCentre, F, sigmaF, 1, 3);

  system.addGaussianPotential(couplingCentre, F, sigmaF, 4, 1);
  system.addGaussianPotential(couplingCentre, F, sigmaF, 1, 4);

  system.addGaussianPotential(couplingCentre, F, sigmaF, 5, 2);
  system.addGaussianPotential(couplingCentre, F, sigmaF, 2, 5);
  system.addGaussianPotential(couplingCentre, F, sigmaF, 6, 2);
  system.addGaussianPotential(couplingCentre, F, sigmaF, 2, 6);

  for (int k = 2*3+1; k < 2*3+3; ++k) {
    system.addGaussianPotential(couplingCentre, F, sigmaF, k, 3);
    system.addGaussianPotential(couplingCentre, F, sigmaF, 3, k);
  }
  for (int k = 2*4+1; k < 2*4+3; ++k) {
    system.addGaussianPotential(couplingCentre, F, sigmaF, k, 4);
    system.addGaussianPotential(couplingCentre, F, sigmaF, 4, k);
  }
  for (int k = 2*5+1; k < 2*5+3; ++k) {
    system.addGaussianPotential(couplingCentre, F, sigmaF, k, 5);
    system.addGaussianPotential(couplingCentre, F, sigmaF, 5, k);
  }
  for (int k = 2*6+1; k < 2*6+3; ++k) {
    system.addGaussianPotential(couplingCentre, F, sigmaF, k, 6);
    system.addGaussianPotential(couplingCentre, F, sigmaF, 6, k);
  }
