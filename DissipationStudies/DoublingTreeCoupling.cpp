...
//    }
    for (int j = 1; j < N; ++j) {
      for (int l = 2*j-1; l < 2*j+1; ++l){
        system.addGaussianPotential(0.0, V0, sigma, l, l);
      }
    }
...
//    }
    for (int j = 1; j < N; ++j) {
      for (int l = 2*j-1; l < 2*j+1; ++l){
        system.addGaussianPotential(couplingCentre, F, sigmaF, j-1, l);
        system.addGaussianPotential(couplingCentre, F, sigmaF, l, j-1);
      }
    }
